...
use shakmaty::{CastlingSide, Chess, Color, File, Position, Rank, Role, Square};

use crate::agents::Agent;
use crate::chess::moves::parse_uci;
use crate::chess::{GameState, Move};

// ── Polyglot Random64 table (781 values) ─────────────────────────────────────
//
// Layout:
//   [0..768]   piece keys: piece_idx * 64 + sq_idx
//              sq_idx = rank * 8 + file  (A1=0, H8=63)
//              piece_idx: bP=0 wP=1 bN=2 wN=3 bB=4 wB=5 bR=6 wR=7 bQ=8 wQ=9 bK=10 wK=11
//   [768..772] castling: White-KS, White-QS, Black-KS, Black-QS
//   [772..780] en-passant file A..H
//   [780]      side-to-move (XOR when White to move)
//
// Source: python-chess POLYGLOT_RANDOM_ARRAY (matches Polyglot 1.4 random.h)
...
fn piece_key_index(role: Role, color: Color) -> usize {
    ...
    // White=1, Black=0  (interleaved with piece type)
    if color == Color::White { role_base + 1 } else { role_base }
}

fn polyglot_hash(pos: &Chess) -> u64 {
    let mut h = 0u64;

    // Pieces
    for (sq, piece) in pos.board().iter() {
        let sq_idx = usize::from(sq.rank()) * 8 + usize::from(sq.file());
        let pidx = piece_key_index(piece.role, piece.color);
        h ^= POLYGLOT_KEYS[pidx * 64 + sq_idx];
    }

    // Castling rights
    let castles = pos.castles();
    if castles.has(Color::White, CastlingSide::KingSide)  { h ^= POLYGLOT_KEYS[768]; }
    if castles.has(Color::White, CastlingSide::QueenSide) { h ^= POLYGLOT_KEYS[769]; }
    if castles.has(Color::Black, CastlingSide::KingSide)  { h ^= POLYGLOT_KEYS[770]; }
    if castles.has(Color::Black, CastlingSide::QueenSide) { h ^= POLYGLOT_KEYS[771]; }

    // En passant (only when a legal capture exists)
    if let Some(ep_sq) = pos.legal_ep_square() {
        h ^= POLYGLOT_KEYS[772 + usize::from(ep_sq.file())];
    }
...
impl OpeningBook {
    /// Load a Polyglot .bin file. The file must be pre-sorted by hash key.
    pub fn load(path: &Path) -> anyhow::Result<Self> {
        let data = fs::read(path)
            .with_context(|| format!("cannot read opening book '{}'", path.display()))?;
        if data.len() % 16 != 0 {
            bail!(
                "opening book '{}' has size {} which is not a multiple of 16",
                path.display(),
                data.len()
            );
        }
        ...
        for chunk in data.chunks_exact(16) {
            ...
            // bytes 12..16 = "learn" u32, discarded
            entries.push(PolyglotEntry { key, mv, weight });
        }
        Ok(Self { entries })
    }

    /// Return all book moves for `pos` with their weights.
    pub fn lookup(&self, pos: &Chess) -> Vec<(Move, u16)> {
        ...
        for entry in &self.entries[start..] {
            ...
            if let Some(mv) = decode_polyglot_move(entry.mv, pos) {
                result.push((mv, entry.weight));
            }
        }
        result
    }
...
// ── Move decoding ─────────────────────────────────────────────────────────────
//
// Bit layout of the 16-bit move field:
//   bits  0-2  from file (0=A)
//   bits  3-5  from rank (0=rank 1)
//   bits  6-8  to file
//   bits  9-11 to rank
//   bits 12-14 promotion: 0=none 1=N 2=B 3=R 4=Q

fn decode_polyglot_move(encoded: u16, pos: &Chess) -> Option<Move> {
    let from_file = (encoded        & 0b111) as u8;
    let from_rank = ((encoded >> 3) & 0b111) as u8;
    let to_file   = ((encoded >> 6) & 0b111) as u8;
    let to_rank   = ((encoded >> 9) & 0b111) as u8;
    let promo     = (encoded >> 12) & 0b111;

    // Polyglot castling uses king→rook-square; shakmaty uses king→king-target.
    let (to_file, to_rank) = match (from_file, from_rank, to_file, to_rank) {
        (4, 0, 7, 0) => (6, 0), // e1h1 → e1g1  (White K-side)
        (4, 0, 0, 0) => (2, 0), // e1a1 → e1c1  (White Q-side)
        (4, 7, 7, 7) => (6, 7), // e8h8 → e8g8  (Black K-side)
        (4, 7, 0, 7) => (2, 7), // e8a8 → e8c8  (Black Q-side)
        _            => (to_file, to_rank),
    };

    let from_sq = Square::from_coords(File::new(from_file as u32), Rank::new(from_rank as u32));
    let to_sq   = Square::from_coords(File::new(to_file   as u32), Rank::new(to_rank   as u32));
    ...
    let uci_str = if let Some(pc) = promo_char {
        format!("{from_sq}{to_sq}{pc}")
    } else {
        format!("{from_sq}{to_sq}")
    };

    parse_uci(pos, &uci_str).ok()
}

// ── Weighted random selection ─────────────────────────────────────────────────

fn pick_weighted(candidates: &[(Move, u16)]) -> Option<Move> {
    ...
    if total == 0 {
        ...
        return candidates.choose(&mut rng).map(|(mv, _)| mv.clone());
    }
    let weights: Vec<u16> = candidates.iter().map(|(_, w)| *w).collect();
    let dist = WeightedIndex::new(&weights).ok()?;
    Some(candidates[dist.sample(&mut rng)].0.clone())
}
...
impl BookAgent {
    pub fn new(inner: Box<dyn Agent + Send>, book: OpeningBook) -> Self {
        Self { inner, book, out_of_book: false }
    }
}
...
impl Agent for BookAgent {
    fn select_move(&mut self, state: &GameState, time_budget: Option<Duration>) -> Move {
        if !self.out_of_book {
            let candidates = self.book.lookup(&state.position);
            if let Some(mv) = pick_weighted(&candidates) {
                return mv;
            }
            self.out_of_book = true;
        }
        self.inner.select_move(state, time_budget)
    }
...
mod tests {
    ...
    /// Mandatory correctness gate: the Polyglot hash of the starting position
    /// must equal the canonical value 0x463b96181691fc9c.
    #[test]
    fn polyglot_hash_start_position() {
        let pos = Chess::default();
        assert_eq!(
            polyglot_hash(&pos),
            0x463b96181691fc9c,
            "Polyglot hash of starting position is wrong"
        );
    }

    /// e2e4 (encoded 0x1014) should decode to a legal pawn push.
    #[test]
    fn decode_normal_pawn_push() {
        let pos = Chess::default();
        // e2e4: from_file=4, from_rank=1, to_file=4, to_rank=3 → bits = 4|(1<<3)|(4<<6)|(3<<9) = 0x0D8C
        let from_file = 4u16;
        let from_rank = 1u16;
        let to_file   = 4u16;
        let to_rank   = 3u16;
        let encoded = from_file | (from_rank << 3) | (to_file << 6) | (to_rank << 9);
        let mv = decode_polyglot_move(encoded, &pos);
        assert!(mv.is_some(), "e2e4 should decode to a legal move");
        assert_eq!(format!("{}", shakmaty::uci::UciMove::from_standard(&mv.unwrap())), "e2e4");
    }

    /// Polyglot encodes White K-side castling as e1h1; we must remap to e1g1.
    #[test]
    fn decode_castling_kingside_white() {
        // Build a position where White can castle K-side.
        use shakmaty::fen::Fen;
        let fen: Fen = "r1bqk2r/pppp1ppp/2n2n2/2b1p3/2B1P3/5N2/PPPP1PPP/RNBQK2R w KQkq - 4 4"
            .parse()
            .unwrap();
        let pos: Chess = fen.into_position(shakmaty::CastlingMode::Standard).unwrap();
        // e1h1 encoding: from=(4,0) to=(7,0)
        let encoded: u16 = 4 | (0 << 3) | (7 << 6) | (0 << 9);
        let mv = decode_polyglot_move(encoded, &pos);
        assert!(mv.is_some(), "e1h1 should remap to legal castling move");
        // The castling move should be e1g1 in UCI
        let uci = shakmaty::uci::UciMove::from_standard(&mv.unwrap()).to_string();
        assert_eq!(uci, "e1g1", "K-side castling should be e1g1, got {uci}");
    }

    /// An empty book returns no candidates; BookAgent falls back to inner agent.
    #[test]
    fn book_agent_fallback_empty_book() {
        let inner = Box::new(RandomAgent::default());
        let book  = OpeningBook { entries: vec![] };
        let mut agent = BookAgent::new(inner, book);
        let state = GameState::new();
        let mv = agent.select_move(&state, None);
        let legal = state.legal_moves();
        assert!(legal.contains(&mv), "fallback move must be legal");
    }

    /// After one book miss, out_of_book is true; further calls skip lookup.
    #[test]
    fn out_of_book_flag_set_after_miss() {
        let inner = Box::new(RandomAgent::default());
        let book  = OpeningBook { entries: vec![] };
        let mut agent = BookAgent::new(inner, book);
        let state = GameState::new();
        // First call → miss (empty book) → out_of_book becomes true
        agent.select_move(&state, None);
        assert!(agent.out_of_book, "out_of_book should be set after a miss");
        // Second call should still work (delegates to inner)
        let mv = agent.select_move(&state, None);
        let legal = state.legal_moves();
